...
return {
	entry = function(_, job)
		...
		local temp_dir_url, temp_dir_err = fs.unique("dir", Url(temp_dir))
		if not temp_dir_url then
			-- Notify the user if the temporary directory creation fails
			notify(string.format("Failed to create temp directory, error: %s", tostring(temp_dir_err)), "error")
			return
		end
		temp_dir = tostring(temp_dir_url)
